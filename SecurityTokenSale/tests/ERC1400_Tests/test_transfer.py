...
def test_lockup(ensemble,accounts):
    ...
    with brownie.reverts("55"):
        ensemble['token'].transfer(accounts[0],20)
...
def test_transferToZeroAddress(ensemble,accounts):
    ...
    with brownie.reverts("57"):
        ensemble['token'].transfer(zeroAddress,20)

def test_transferBalance(ensemble,accounts):
    ensemble['token'].unpause({'from':accounts[0]})
    initialize_account(ensemble,[accounts[0],accounts[1],accounts[2]])
    ensemble['token'].transfer(accounts[2],10,{'from':accounts[1]})
...
def test_twoPartitions(ensemble,accounts):
    ...
    ensemble['token'].transfer(accounts[1],200,{'from':accounts[0]})
...
def test_events(ensemble,accounts):
    ...
    ensemble['token'].setDefaultPartitions([b'Holding',b'Partition2'])
    tx = ensemble['token'].transfer(accounts[1],140,{'from':accounts[0]})
    Events.check_transferEvent(tx,0,accounts[0],accounts[1],100)
    Events.check_transferByPartitionEvent(tx,1,b'Holding'.hex(),accounts[0],accounts[0],accounts[1],100,b''.hex(),b''.hex())
    Events.check_transferEvent(tx,2,accounts[0],accounts[1],40)
    Events.check_transferByPartitionEvent(tx,3,b'Partition2'.hex(),accounts[0],accounts[0],accounts[1],40,b''.hex(),b''.hex())